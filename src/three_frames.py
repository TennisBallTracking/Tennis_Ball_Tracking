...
class ConvNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(768, 768, kernel_size=3, padding=1) # the output size can be tuned
    ...
    def forward(self, features: List[torch.Tensor], batch_size):
        ...
        input = features.reshape(batch_size, 768, h, w).to(device)
        x = self.conv1(input)
        x = self.relu(x)
        x = self.conv2(x)
        return x
...
MODEL_FILE = '../models/spatial_temporal_model/ball_model_convnet_3.pth'
...
model = ConvNet()
model.load_state_dict(torch.load('../models/spatial_temporal_model/ball_model_convnet_3.pth', map_location='cuda'))
model.to(device)
model.eval()
...
for index, row in tqdm(df_test.iterrows()):
    ...
    with torch.no_grad():
        ...
        feature_maps = objectness[:, :, :H//4, :W//4]
    fh, fw = feature_maps.shape[-2:]
    feature_maps = torch.reshape(feature_maps, (1, 3, 256, fh, fw))
    with torch.no_grad():
...
result.to_csv('../csv_files/m-%03d-three.csv', index=False)
