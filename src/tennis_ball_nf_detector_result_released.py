...
NUM = 3 # How many consecutive frames
MATCH_ID = 20
DEBUG = False
...
for index, row in tqdm(df_test.iterrows()):
    ...
    if DEBUG:
        cv2.circle(center_frame, (int(x_final), int(y_final)), 10, (0, 0, 255), 3)
        cv2.imshow('frame', center_frame)
        cv2.waitKey(40)
...
result.to_csv('../csv_files/m-%03d-%d-frames.csv'%(NUM), index=False)
